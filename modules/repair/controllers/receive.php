...
class Controller extends \Gcms\Controller
{
    ...
    public function render(Request $request)
    {
        ...
        // ข้อความ title bar
        $this->title = Language::get($index->id == 0 ? 'Get a Technical Service' : 'Technical Service job description');
        // เลือกเมนู
        $this->menu = 'repair';
        ...
        if ($login && ($index->id == 0 || $login['id'] == $index->customer_id || Login::checkPermission($login, 'can_manage_repair'))) {
            ...
            // แสดงฟอร์ม
            $section->appendChild(\Repair\Receive\View::create()->render($login ,$index ));
            // คืนค่า HTML
